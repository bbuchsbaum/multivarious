...
1.  Fit a Behavior-style PLSC model
...
4.  Use bootstrap ratios to identify stable loadings, with simple
    visualizations.

The workflow is intentionally small and fast so the vignette runs
quickly; for real analyses, increase the number of
...
![](PLSC_files/figure-html/scores-plot-1.png)

## 5. Permutation test: how many LVs?

Shuffle rows of `Y` to break the X–Y link and build an empirical null
...
![](PLSC_files/figure-html/bootstrap-1.png)

Do the same for Y loadings if needed:

![](PLSC_files/figure-html/bootstrap-y-1.png)

## 7. Practical tips

- Use
  [`standardize()`](https://bbuchsbaum.github.io/multivarious/reference/standardize.md)
  to run PLSC on correlation scale (default here); switch to
  [`center()`](https://bbuchsbaum.github.io/multivarious/reference/center.md)
  for covariance scale.  
- Increase `nperm` (e.g., 999) and `nboot` (≥500) for publication-grade
  inference.  
- Interpret loadings only where bootstrap ratios exceed ~\|2\|, and only
  for LVs that pass the permutation test.  
- To visualise higher-dimensional loading maps (e.g., imaging), replace
  the bar plots with your spatial plotting routine applied to
  `boot_plsc$z_vx`.
